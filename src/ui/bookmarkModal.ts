...
export class BookmarkModal extends UnreaderModal {
	...
	onOpen(): void {
		...
		this.titleEl.setText(this.options.title ?? "添加书签");
		this.contentEl.empty();

		this.contentEl.createDiv({
			cls: "unreader-tag-desc",
			text: this.options.description ?? "为当前阅读位置添加书签，可填写备注便于识别（可选）。",
		});
		...
		const input = this.contentEl.createEl("input", {
			...
			type: "text",
			attr: { placeholder: this.options.placeholder ?? "备注（可选），如：重点章节" },
		});
