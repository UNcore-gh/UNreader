...
A beautiful e-book reader for Obsidian — **EPUB first**, plus MOBI/AZW3 and TXT. Reflowable layout with a seamless continuous scroll, a floating table of contents, an on-edge chapter rail, highlights / bookmarks / comments, and a full appearance system (fonts, backgrounds, presets).
...
| ** Presets** | Named appearance presets you can switch between, export and reuse across books |
...
| ** Mobile ready** | Swipe from the text area to open Obsidian's side drawer, touch-friendly rail, soft-keyboard aware layout |
...
[MIT](LICENSE) — the compiled plugin (this repository) is MIT licensed. The original source code lives in a private repository and is available to Obsidian's plugin reviewers on request.

---

# 中文说明

**UNreader** 是 Obsidian 的电子书阅读器——**以 EPUB 为主**，同时支持 MOBI/AZW3 与 TXT。重排阅读、无缝连续滚动、浮动目录 + 右缘章节轨、高亮/书签/批注，以及完整的外观系统（字体、背景图、外观预设）。
...
| ** 移动端适配** | 正文区横滑呼出 Obsidian 侧栏，触屏友好的章节轨，软键盘感知布局 |
...
[MIT](LICENSE) —— 本仓库（编译后的插件）采用 MIT 协议；原始源码位于私有仓库，Obsidian 官方审核可按需授权访问。