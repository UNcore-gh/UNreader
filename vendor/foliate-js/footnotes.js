...
export class FootnoteHandler extends EventTarget {
    ...
    handle(book, e) {
        // UNreader patch: allow a pre-resolved target {index, anchor} to be
        // injected via e.detail.target — needed because bare "#fragment" hrefs
        // cannot be resolved by book.resolveHref() (it requires a full path).
        const { a, href, target: preTarget } = e.detail || {}
        const { yes, maybe } = isFootnoteReference(a)
        if (yes) {
            e.preventDefault()
            return Promise.resolve(preTarget ?? book.resolveHref(href)).then(t =>
                this.#showFragment(book, t, href))
        }
        else if (this.detectFootnotes && maybe()) {
            e.preventDefault()
            return Promise.resolve(preTarget ?? book.resolveHref(href)).then(({ index, anchor }) => {
