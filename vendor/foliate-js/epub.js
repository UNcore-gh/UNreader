...
class Loader {
    ...
    async loadReplaced(item, parents = []) {
        ...
        if ([MIME.XHTML, MIME.HTML, MIME.SVG].includes(mediaType)) {
            ...
            // inline stylesheets as <style>: CSP (style-src) blocks blob: linked
            // stylesheets in Obsidian, which drops all book formatting
            for (const el of [...doc.querySelectorAll('link[href]')]) {
                const rel = (el.getAttribute('rel') || '').toLowerCase()
                const lhref = el.getAttribute('href') || ''
                const isCss = rel.includes('stylesheet')
                    || (el.getAttribute('type') || '') === 'text/css'
                    || /\.css(\?|#|$)/i.test(lhref)
                if (isCss) {
                    try {
                        const path = resolveURL(lhref, href)
                        const item = this.manifest.find(x => x.href === path)
                        if (item) {
                            const css = await this.loadText(item.href)
                            const replaced = await this.replaceCSS(css, item.href,
                                parents.concat(href))
                            const style = doc.createElementNS(NS.XHTML, 'style')
                            style.textContent = replaced
                            el.replaceWith(style)
                            continue
                        }
                    } catch (e) {
                        console.warn('Failed to inline stylesheet', lhref, e)
                    }
                }
